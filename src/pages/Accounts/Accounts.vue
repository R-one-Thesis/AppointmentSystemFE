<template>
    <div class="q-pa-md">
      <q-dialog v-model="viewTransaction" persistent transition-show="flip-down">
      <q-card style="width: 1500px; max-width: 700vw; height: 660px; max-height: 120vw;">
        <q-toolbar>
          <!-- <q-avatar>
          ...
          <q-toolbar-title
            ><span class="text-weight-bold"
              >Account Transactions</span
            ></q-toolbar-title
          >
          <q-btn flat round dense icon="close" v-close-popup />
        </q-toolbar>
        <div class="q-pa-md unmodify">
        <q-table
        title="Transactions"
    ...
    <template v-slot:body-cell-type="props">
        ...
        <template v-slot:top-left="props">
          ...
          >
            <!-- Table Title -->
            <q-label class="q-table__title q-mr-md">Accounts List</q-label>
  ...
  const getAccountTransaction = (row) => {
    console.log(row)
      row.loading = true;
    api
      .getAccountTransaction(row.id)
      .then((response) => {
        console.log(response);
      
        tablerowsTransaction.value = response.transactions
        console.log(tablerowsTransaction.value)
  ...
  const loadData = () => {
    ...
    api
      .viewAllAccounts(businessId.value)
      .then((response) => {
        console.log(response);
  ...
  const loadDataFiltered = () => {
    ...
    api
      .viewAllAccounts(selectedBusiness.value.id)
      .then((response) => {
        console.log(response);
        tablerows.value = response.length > 0 ? response : [];
        loading.value = false;
